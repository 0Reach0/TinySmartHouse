...
void update_sta_list(const wifi_event_ap_staconnected_t *event) {
    ...
    uint32_t ip = get_sta_ip_address(event);
    if (ip == 0) {
        return;
    }

    int sock = authorizate(ip);
    if (sock < 0) {
        return;
    }
    ...
    if (recv_len > 0) {
        ...
        add_device_to_list(event, ip, buffer[0]);
    }
...
uint32_t get_sta_ip_address(const wifi_event_ap_staconnected_t *event) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error getting STA list: %s", esp_err_to_name(err));
        return 0;
    }

    for (int i = 0; i < station_list.num; i++) {
        if (memcmp(station_list.sta[i].mac, event->mac, 6) == 0) {
...
void add_device_to_list(const wifi_event_ap_staconnected_t *event, uint32_t ip, uint8_t device_id) {
    ...
    memcpy(devices[device_count].mac, event->mac, 6);
    device_count++;
    ESP_LOGI(TAG, "Device id %i \n", device_id);
    ESP_LOGI(TAG, "Device ip: %i", ip);
}
